def newineq(a,b,A,B,g):
    r"""
    Checks to see if values given for 5 parameters satisfy the UPDATED system of inequalities below, with the last 2 gathered from SPAM on ineq.sage.
    ...
    EXAMPLES::
        ...
        sage: newineq(2,1,1,1/2,10)
        True
        
        sage: newineq(1,1,1,1,1)
        False
    ...
    if not(((2*adif)/(3*b))*(bg45/(bg45-(b2dif*b1sum)))):
        #print("ie4 bad")
        checker.append("")
